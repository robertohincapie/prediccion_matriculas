...
def validate_course_codes(df: pd.DataFrame) -> None:
    ...
    if df["course_code"].isna().any():
        raise DataValidationError(
            "Existen registros sin código de curso."
        )
    ...
    if empty.any():
        raise DataValidationError(
            "Existen códigos de curso vacíos."
        )
...
def validate_enrollment(df: pd.DataFrame) -> None:
    ...
    if enrollment.isna().any():
        raise DataValidationError(
            "Existen valores de matrícula no numéricos o nulos."
        )

    if (enrollment < 0).any():
        raise DataValidationError(
            "Existen valores de matrícula negativos."
        )
...
def validate_semesters(df: pd.DataFrame) -> None:
    ...
    if invalid.any():
        ...
        raise DataValidationError(
            f"Existen semestres inválidos: {values}"
        )
...
def validate_years(df: pd.DataFrame) -> None:
    ...
    if df["year"].isna().any():
        raise DataValidationError(
            "Existen registros sin año."
        )

    if (df["year"] < BASE_YEAR).any():
        raise DataValidationError(
            f"Existen años anteriores a BASE_YEAR={BASE_YEAR}."
        )
...
def validate_time_index(df: pd.DataFrame) -> None:
    ...
    if inconsistent.any():
        raise DataValidationError(
            "Existen índices temporales inconsistentes."
        )
